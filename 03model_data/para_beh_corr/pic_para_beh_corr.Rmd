...
#0. load the csv data
```{r}
all_data                      <- fread(file="para_beh_corr.csv")
```
...
scatterplot2 <- function(d, xlimits, xbreaks, ylimits, ybreaks, xlabs, ylabs) {
  ...
  fig <- ggplot(d, aes(x = x, y = y)) +
    ...
    theme(
      ...
      axis.text.x = element_text(size = size1, colour = '#4D4D4D', family = "Arial"),
      axis.text.y = element_text(size = size1, colour = '#4D4D4D', family = "Arial"),
      axis.title = element_text(size = size1, colour = 'black', family = "Arial"),
    )+
  theme(
...
ggsave('./Pic/beta_UF.png', 
       ...
       dpi = 300)
# 计算相关性
cor.test(all_data$beta_outGvsinG, all_data$UFout_in)
